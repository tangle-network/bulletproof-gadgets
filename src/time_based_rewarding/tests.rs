use super::{
	AllocatedInputCoin, AllocatedOutputCoin, AllocatedTimedDeposit, Transaction,
};
use crate::{
	poseidon::{
...
fn test_time_based_reward_gadget_verification() {
	...
	let mut deposit_tree = SparseMerkleTreeBuilder::new()
	...
	assert!(deposit_tree.verify_proof(
		k,
		expected_output,
		&merkle_proof_vec,
		None
	...
	let timed_deposit_leaf_val =
		Poseidon_hash_2(expected_output, deposit_block_number, &p_params);

	let mut timed_tree = SparseMerkleTreeBuilder::new()
		.hash_params(p_params.clone())
		.build();
	...
	let _output_1_sn =
		Poseidon_hash_2(output_1_r, output_1_nullifier, &p_params);
	...
	let _output_2_sn =
		Poseidon_hash_2(output_2_r, output_2_nullifier, &p_params);
	...
	let (proof, commitments) = {
		...
		let (com_input_r, var_input_r) =
			prover.commit(r.clone(), Scalar::random(&mut test_rng));
		...
		let (com_deposit_time, var_deposit_time) = prover.commit(
			deposit_block_number.clone(),
		...
		let (com_deposit_time_leaf_val, var_deposit_time_leaf_val) = prover
			.commit(
				timed_deposit_leaf_val.clone(),
				Scalar::random(&mut test_rng),
			);
		...
		let (com_output_1_inverse_val, var_output_1_inverse_val) = prover
			.commit(output_1_inverse.clone(), Scalar::random(&mut test_rng));
		...
		let (com_output_1_nullifier, var_output_1_nullifier) = prover
			.commit(output_1_nullifier.clone(), Scalar::random(&mut test_rng));
		...
		let (com_output_2_inverse_val, var_output_2_inverse_val) = prover
			.commit(output_2_inverse.clone(), Scalar::random(&mut test_rng));
		...
		let (com_output_2_nullifier, var_output_2_nullifier) = prover
			.commit(output_2_nullifier.clone(), Scalar::random(&mut test_rng));
		...
		(
			...
			(
				...
				[
					timed_comms,
					deposit_time_index_comms,
					deposit_time_proof_comms,
				],
				output_comms,
			),
		)
